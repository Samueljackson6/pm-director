...
**📌 数据层级澄清（2026-07-09）**：
- **PDF 合同扫描件 = 源头真值**（影像，无文本层）
- **DOCX = 手动 OCR 后的文本层**（用户用软件把 PDF 扫描件转成，派生，用于抽取）
- **`cache/contracts/*.json` = 从 DOCX 再抽取的结构化中间产物**（非合同全文）

> 存量 50+ 合同已是你手动 OCR 为 DOCX 的既成事实；下方 PP-StructureV3 流程仅用于**新增合同**的自动入库。条款级分析（违约/罚款等）必须回到 DOCX / PDF，不能只用 JSON。
