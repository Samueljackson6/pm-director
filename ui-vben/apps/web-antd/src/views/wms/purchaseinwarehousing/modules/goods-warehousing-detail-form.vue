...
import { getGoodsWarehousingDetailListByPurchaseOrderId } from '#/api/wms/purchaseinwarehousing';
import { FileUpload } from '#/components/upload';
import { $t } from '#/locales';

import { useGoodsWarehousingDetailGridEditColumns } from '../list/data';
...
/** 新增明细 */
const onAdd = async () => {
  await gridApi.grid.insertAt({} as PurchaseInWarehousingApi.GoodsWarehousingDetail, -1);
};

defineExpose({
  getData: (): PurchaseInWarehousingApi.GoodsWarehousingDetail[] => {
    const data =
      gridApi.grid.getData() as PurchaseInWarehousingApi.GoodsWarehousingDetail[];
    const removeRecords =
      gridApi.grid.getRemoveRecords() as PurchaseInWarehousingApi.GoodsWarehousingDetail[];
    const insertRecords =
...
watch(
  ...
  async (val) => {
    ...
    await gridApi.grid.loadData(
      await getGoodsWarehousingDetailListByPurchaseOrderId(
        props.purchaseOrderId!,
      ),
    );
  },
...
<template>
  <Grid class="mx-4">
    ...
    <template #commonOperationId="{ row }">
      <Input v-model:value="row.commonOperationId" />
    </template>
  ...
  <div class="-mt-4 flex justify-center">
    <Button
      ...
      ghost
      @click="onAdd"
      v-access:code="['wms:purchase-in-warehousing:create']"
    >
      {{
