  ...
  <div class="pm-workbench-page min-h-screen p-4 sm:p-6">
    ...
    <a-card class="pm-table-surface" :body-style="{ padding: 0 }" aria-label="供应商付款列表">
      <div class="p-4 border-b border-gray-100">
        <h3 class="text-base font-semibold">付款列表</h3>
        <p class="mt-1 text-xs text-gray-500">仅展示供应商付款记录；客户回款与供应商收票分别管理，不会混入本列表。</p>
      </div>
      <div class="p-4">
        ...
        <a-table :data-source="list" :columns="columns" :loading="loading" row-key="payment_id" size="middle"
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'status'">
              <a-tag :color="record.status === '已付款' ? 'green' : record.status === '已完' ? 'green' : 'orange'">
                {{ record.status }}
              </a-tag>
            </template>
            <template v-if="column.key === 'linked_invoices'">
              <a-tag v-if="record._linked_count && record._linked_count > 0" color="blue">
                {{ record._linked_count }} 张发票
              </a-tag>
    ...
    <a-modal v-model:open="modalVisible" :title="editingId ? '编辑付款' : '新增付款'" :confirm-loading="saving" @ok="saveRecord" width="600px">
      <a-form :model="form" layout="vertical">
        <a-row :gutter="16">
          ...
          <a-col :span="12"><a-form-item label="状态"><a-select v-model:value="form.status"><a-select-option value="已付款">已付款</a-select-option><a-select-option value="待支付">待支付</a-select-option></a-select></a-form-item></a-col>
...
const columns = [
  ...
  { title: '项目编号', dataIndex: 'project_id', key: 'project_id', width: 180, ellipsis: true },
  ...
  { title: '关联发票', dataIndex: 'linked_invoices', key: 'linked_invoices', width: 100 },
...
async function load() {
  ...
  try {
    ...
    for (const item of list.value) {
      try {
        const res: any = await requestClient.get(`/api/suppliers/payments/${item.payment_id}`)
        const linked = res.data?.linked_invoices ?? res.linked_invoices ?? []
        item._linked_count = linked.length
      } catch {
        item._linked_count = 0
      }
    }
  } catch { /* ignore */ } finally { loading.value = false }
}
function formatAmount(value: number | null | undefined) { return value == null ? '待核实' : Number(value).toFixed(2) }
...
function openAdd() {
  ...
  form.value = {
    project_id: '', payment_date: '', amount: null, payment_method: '银行转账', status: '已付款', notes: ''
  }
